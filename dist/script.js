let cont=0;
let au = new Audio();
var titu="Aprovecha con letra - Nova Jory Ft. Daddy Yankee.mp3";
au.src = 'AUDIO/'+titu;
...
// ===== Curation Page  =====
// ===== List  =====
$('.item, .itemsalsa, .itembalada').hover(function(){
...
$('.text-wrap .text').click(function(){
	...
	// Background down
	homeToMain.to($('.wave-container'), 1, {yPercent: 100, ease: Power2.easeInOut}, 0),
...
$('.itembalada').click(function(){
	window.location.href = 'balada.php';
});
$('.itemsalsa').click(function(){
	window.location.href = 'salsa.php';
});
...
// ===== From Playlist(3) to Main(2)
	if($('#curator').css("display") == "none"){
		...
		mainToHome.to($('#upload'), 0, {display: 'none', ease: Power2.easeInOut}, 1),
		
		mainToHome.to($('#salsa'), 0, {display: 'none', ease: Power2.easeInOut}, 1),
		
		mainToHome.to($('#balada'), 0, {display: 'none', ease: Power2.easeInOut}, 1),
